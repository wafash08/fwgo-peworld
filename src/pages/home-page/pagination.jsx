...
export default function Pagination() {
	return (
		<ul className='flex items-center justify-center gap-4'>
			<PaginationItem label='sebelumnya' />
			<PaginationItem label='1' current />
			<PaginationItem label='2' />
			<PaginationItem label='3' />
			<PaginationItem label='selanjutnya' />
		</ul>
	);
}

function PaginationItem({ label, current }) {
	let icon = null;
	switch (label) {
		case 'sebelumnya': {
			icon = (
			...
			<button
				...
				className={clsx(
					...
					current
						...
						: 'bg-white text-quick-silver border-azureish-white'
				)}
			>
				{icon ? <span className='sr-only'>{label}</span> : <span>{label}</span>}
				{icon}
			</button>
		</li>
	);
